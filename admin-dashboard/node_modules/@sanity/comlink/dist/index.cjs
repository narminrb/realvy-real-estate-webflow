...
const listenInputFromContext = (config) => ({
  ...
  const { targetOrigin } = input, targets = /* @__PURE__ */ new Set(), connections = /* @__PURE__ */ new Set();
  return {
    addTarget: (target) => {
    ...
    createConnection: (input2, machine = createChannelMachine()) => {
      ...
      const post = (data) => {
        ...
        const channels2 = connection.channels;
        channels2.forEach(cleanupChannel), channels2.clear(), connections.delete(connection);
      };
      return {
        on,
        onInternalEvent,
    ...
    destroy: () => {
      connections.forEach(({ channels }) => {
        channels.forEach(cleanupChannel), channels.clear();
      }), connections.clear(), targets.clear();
    }
  };
  ...
  actions: {
    ...
    "set connection config": xstate.assign({
      connectionId: ({ event }) => (xstate.assertEvent(event, "handshake.syn"), event.message.data.connectionId),
