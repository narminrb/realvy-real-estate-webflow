...
function useRouterPanesStream() {
  const routerStateSubject = useMemo(() => new ReplaySubject(1), []), routerPanes$ = useMemo(() => routerStateSubject.asObservable().pipe(map((_routerState) => _routerState?.panes || [])), [routerStateSubject]), {
    state: routerState
  } = useRouter();
  return useEffect(() => {
